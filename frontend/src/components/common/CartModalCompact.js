...
import AddressFormEnhanced from './AddressFormEnhanced';
import PaymentOptions from './PaymentOptions';

const CartModalCompact = () => {
  const {
    items,
    total,
    itemCount,
  ...
  const canProceed = () => {
    switch (currentStep) {
      ...
      case 3: return address.street && address.area && address.city && address.zipCode;
  ...
  return (
    <div className={`cart-modal-overlay ${animationClass}`} onClick={handleBackdropClick}>
      <div className="cart-modal-compact">
        {/* Header */}
        <div className="cart-modal-header-compact">
          <div className="cart-modal-title-compact">
            <h2 className="text-xs font-bold text-gray-800">{getStepTitle()}</h2>
            <p className="text-gray-600 text-xs mt-0.5">{getStepDescription()}</p>
          </div>
          <button
            onClick={handleClose}
            className="cart-modal-close-compact"
          >
        ...
        {/* Progress Bar */}
        <div className="cart-modal-progress-compact">
          ...
          <div className="progress-steps-compact">
            {[1, 2, 3, 4, 5].map((step) => (
              <div
                key={step}
                className={`progress-step-compact ${step <= currentStep ? 'active' : ''} ${step < currentStep ? 'completed' : ''}`}
              >
                <span className="step-number-compact">{step}</span>
        ...
        {/* Content */}
        <div className="cart-modal-content-compact">
          {currentStep === 1 && (
            <div className="cart-items-section-compact">
              {items.length === 0 ? (
                <div className="empty-cart-compact">
                  <i className="fas fa-shopping-cart text-xl text-gray-300 mb-1"></i>
                  <h3 className="text-xs font-semibold text-gray-600 mb-0.5">Your cart is empty</h3>
                  <p className="text-gray-500 text-xs">Add some items to get started!</p>
                </div>
              ) : (
                <>
                  <div className="cart-items-list-compact">
                    {items.map((item) => (
                  ...
                  <div className="cart-summary-compact">
                    ...
                    <div className="summary-row-compact total-compact">
                      <span>Total</span>
                      <span>${total.toFixed(2)}</span>
                    </div>
                  </div>
          ...
          {currentStep === 3 && <AddressFormEnhanced />}
          {currentStep === 4 && <PaymentOptions />}
          {currentStep === 5 && (
            <div className="order-confirmation-compact">
              <div className="confirmation-icon-compact">
                <i className="fas fa-check-circle text-xl text-green-500"></i>
              </div>
              <h3 className="text-xs font-semibold text-gray-800 mb-0.5">Order Confirmed!</h3>
        ...
        {/* Footer */}
        <div className="cart-modal-footer-compact">
      ...
      <style jsx>{`
        ...
        .animate-out .cart-modal-compact {
        ...
        .cart-modal-title-compact h2 {
          margin: 0;
          font-size: 0.75rem;
        }

        .cart-modal-close-compact {
        ...
        .cart-modal-close-compact:hover {
          background: rgba(255, 255, 255, 0.3);
          transform: scale(1.1);
        }

        .cart-modal-progress-compact {
        ...
        .progress-step-compact.active .step-label-compact {
          color: #667eea;
        }

        .cart-modal-content-compact {
          flex: 1;
          overflow-y: auto;
          padding: 0.375rem;
          min-height: 80px;
        ...
        .summary-row-compact.total-compact {
...
export default CartModalCompact;
