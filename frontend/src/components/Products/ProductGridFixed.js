...
const ProductGrid = () => {
  ...
  const { toggleFavorite, isFavorite, items: favorites } = useFavorites();
  ...
  const handleAddToCart = (product) => {
    ...
    // Open checkout modal after adding to cart
    setTimeout(() => {
      openCheckout();
    }, 500);
  };
  ...
  const handleToggleFavorite = (product, e) => {
    ...
    // Show toast notification after a small delay to allow state to update
    setTimeout(() => {
            ...
            >
              ...
              <div className="relative overflow-hidden bg-gray-50">
                ...
                <div className="absolute top-4 right-4 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-x-4 group-hover:translate-x-0">
                  ...
                  >
                    <i className={`fas fa-heart transition-all duration-300 ${animatingHearts.has(product.id) ? 'animate-pulse' : ''}`}></i>
